...
arrow::Result<uint64_t> SumPrice(const arrow::Table& table) {
  ARROW_ASSIGN_OR_RAISE(auto sum,
                        arrow::compute::Sum({table.GetColumnByName("price")}));
...
arrow::Result<std::shared_ptr<arrow::Table>> SumGroupByPriority(
  ...
  for (auto i = 0; i < groups->length(); i++) {
    ...
    ARROW_RETURN_NOT_OK(
        price_builder.Append(sum.scalar_as<arrow::UInt64Scalar>().value));
  }
  std::shared_ptr<arrow::Array> price_array;
  ARROW_RETURN_NOT_OK(price_builder.Finish(&price_array));
  ...
  auto result = arrow::Table::Make(result_schema,
                                   {priority_batch->column(0), price_array});
  return result;
}
